...
policy-3rdparty = WebExtensions が chrome.storage.managed 経由でアクセスできるポリシーを設定します。

policy-AllowedDomainsForApps = Google Workspace へのアクセスを許可するドメインを定義します。

policy-AllowFileSelectionDialogs = ファイル選択ダイアログの使用を許可します。

policy-AppAutoUpdate = アプリケーションの自動更新を有効または無効にします。

policy-AppUpdatePin = { -brand-short-name } が指定されたバージョン以上に更新されないようにします。

policy-AppUpdateURL = カスタムアプリの更新 URLを設定します。
...
policy-BlockAboutAddons = アドオンマネージャー (about:addons) へのアクセスはブロックされています。

policy-BlockAboutConfig = about:config ページへのアクセスはブロックされています。

policy-BlockAboutProfiles = about:profiles ページへのアクセスはブロックされています。

policy-BlockAboutSupport = about:support ページへのアクセスはブロックされています。
...
policy-SearchEngines = 検索エンジンの設定を構成します。 このポリシーは、延長サポート リリース (ESR) バージョンでのみ利用可能です。

policy-SearchSuggestEnabled = 検索候補の表示を有効または無効にします。

# For more information, see https://wikipedia.org/wiki/PKCS_11
policy-SecurityDevices2 = PKCS #11 モジュールの追加または削除します。。

policy-ShowHomeButton = ツールバーにホームボタンを表示します。

policy-SSLVersionMax = 最大の SSL バージョンを指定します。

policy-SSLVersionMin = 最小の SSL バージョンを指定します。

policy-StartDownloadsInTempDirectory = デフォルトのダウンロード ディレクトリではなく、ローカルの一時的な場所でダウンロードを強制的に開始します。

policy-SupportMenu = [ヘルプ] メニューに[カスタムサポート] メニューを追加します。
...
policy-WebsiteFilter = ウェブサイトへのアクセスをブロックします。フォーマットの詳細については、ドキュメントを参照してください。

policy-Windows10SSO = Microsoft、職場、学校のアカウントに対して Windows シングル サインオンを許可します。
