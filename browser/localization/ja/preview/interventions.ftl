...
intervention-refresh-profile = 最適なパフォーマンスを得るには、デフォルトの設定を復元し、古いアドオンを削除する必要があります。
intervention-refresh-profile-confirm = { -brand-short-name } をリフレッシュする…
...
intervention-update-ask = 新しいバージョンの { -brand-short-name } が利用できます。
intervention-update-ask-confirm = インストールと再起動をして更新する

## Shown when Firefox does not need to update so instead we offer to refresh
## the user's profile.

intervention-update-refresh = 最新の { -brand-short-name } を使用しています。問題を解決しようとしていますか?。デフォルトの設定を復元し、古いアドオンを削除することで最適なパフォーマンスを得ることができます。
intervention-update-refresh-confirm = { -brand-short-name } をリフレッシュする…
...
intervention-update-web = 最新の { -brand-short-name } ブラウザーを入手する。
intervention-update-web-confirm = 今すぐダウンロード
