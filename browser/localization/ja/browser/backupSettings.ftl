...
settings-data-backup-header2 =
    .label = バックアップ
    .description = ブックマーク、履歴、その他のデータを自動的に保護します。
...
settings-data-backup-in-progress-button = バックアップを実行中…
settings-data-backup-in-progress-message =
    .message = バックアップを実行中…
settings-data-backup-last-backup-date = 前回のバックアップ: { DATETIME($date, dateStyle: "short") } { DATETIME($date, timeStyle: "short") }
...
settings-data-backup-restore-scheduled-on =
    .label = データを復元
    .description = 最後にバックアップされた時点の状態に { -brand-product-name } のデータを復旧します。
...
settings-data-toggle-encryption-label2 =
    .label = 機密データをバックアップする
    .description = パスワードや支払い方法をバックアップし、さらに暗号化によってすべてのデータを安全に保護します。
settings-data-toggle-encryption-support-link = 詳細はこちら
...
turn-on-scheduled-backups-error-default-dir-denied = バックアップフォルダーにアクセスできませんでした。別の保存先を選択してください。
...
restore-from-backup-type-replace =
    .label = すべてのデータをこのバックアップで置き換える
restore-from-backup-type-add =
    .label = データを保持して新しいプロファイルを作成する
restore-from-backup-profiles-disabled-message = 現在のすべての { -brand-short-name } データがバックアップデータで置き換えられます。
...
password-rules-email-description2 = メールアドレスではありません
...
backup-restored-profile-name = { $deviceName } から復元 ({ DATETIME($date, year: "numeric", month: "numeric", day: "numeric") } { DATETIME($date, hour: "2-digit", minute: "2-digit") })

backup-file-restore-file-validation-error = このファイルは正しく読み込めませんでした。別のファイルを選択してください。<a data-l10n-name="restore-problems">まだ問題が解決しませんか？</a>
