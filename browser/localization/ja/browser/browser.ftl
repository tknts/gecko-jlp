...
urlbar-identity-button2 =
    .aria-label =
        { $count ->
            [0] サイトの情報を表示
            [1] サイトの情報を表示 (トラッカーをブロックしました)
           *[other] サイトの情報を表示 ({ $count } 個のトラッカーをブロックしました)
        }
...
urlbar-searchmode-button3 =
    .title = 現在の検索エンジンは { $engine } です。選択してください
urlbar-searchmode-button-no-engine2 =
    .title = ショートカットが選択されていません。選択してください
...
urlbar-searchmode-popup-add-engine = “{ $engineName }” を追加
    .title = 検索エンジン “{ $engineName }” を追加

# Label shown on the top of Searchmode Switcher popup. After this label, the
# available search engines will be listed.
urlbar-searchmode-popup-one-off-header = 今回の検索に使用するエンジン:
...
fullscreen-keyboardlock-exit-button = 全画面表示を終了 ([Esc] キーを長押し)
# "esc" is lowercase on mac keyboards, but uppercase elsewhere.
fullscreen-keyboardlock-exit-mac-button = 全画面表示を終了 ([esc] キーを長押し)
