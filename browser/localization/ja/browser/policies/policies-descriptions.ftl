...
policy-Containers = コンテナ タブに関するポリシーを設定します。

policy-DisableAccounts = 同期などのアカウントが必要なサービスを無効にします。

policy-DisabledCiphers = 暗号を無効にします。

policy-DefaultBrowserSettingEnabled = ユーザーが { -brand-short-name } をデフォルトのブラウザーに設定できるかどうかの制御

policy-DefaultDownloadDirectory = デフォルトのダウンロード先を設定します。

policy-DefaultSerialGuardSetting = Serial API の使用の制御

policy-DisableAppUpdate = ブラウザーの更新を禁止します。
...
policy-DisableRemoteImprovements = アップデートの合間に、{ -brand-short-name } がパフォーマンス、安定性、および機能の変更を適用するのを防ぎます。

policy-DisableRemoteSettingsAndAcceptSecurityConsequences = リモート設定を無効にします。
...
policy-PopupBlocking2 = 特定のウェブサイトがポップアップを表示したり、サードパーティーのフレームによるリダイレクトを行ったりすることを許可します。
