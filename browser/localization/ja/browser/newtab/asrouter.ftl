...
launch-on-login-autostart-infobar-message = Windows へのサインイン時に { -brand-short-name } が起動するようになりました。この設定はいつでも設定画面から変更できます。
launch-on-login-autostart-infobar-keep-button = 有効のままにする
  .accesskey = K
...
welcome-back-spotlight-title = 組み込みのプライバシー機能で再び快適に
welcome-back-spotlight-subtitle = 非営利団体が支援する唯一の主要ブラウザーへおかえりなさい。私たちは、あなたがどこを閲覧していても、データを保護するために特別な対策を講じています。
welcome-back-embedded-import-title = データをインポートして { -brand-short-name } を使い慣れた環境に
...
etp-strict-exceptions-infobar-message = <strong>厳格なトラッキング防止により、サイトが機能しなくなる可能性があります。</strong> トラッカーを含む可能性のある必須要素のブロックを解除することで、一般的な問題を修正します。
...
nova-early-access-infobar-title = <strong>{ -brand-product-name } が新しくなります。</strong> 今年後半の正式リリースに先駆けて、初期の未完成バージョンをプレビューしています。

nova-early-access-share-feedback-link = フィードバックを送信
  .accesskey = S
...
refresh-unused-profile-infobar-message = しばらく { -brand-short-name } を起動されていないようですね。新品同様の快適な状態で使えるよう、クリーンアップしませんか? おかえりなさい!
refresh-reinstalled-profile-infobar-message = Firefoxを再インストールされたようですね。新品同様の快適な状態で使えるよう、クリーンアップしませんか?
