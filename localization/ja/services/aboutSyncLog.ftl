...
about-sync-log-empty-filtered = 現在のフィルターに一致するログはありません。
...
about-sync-log-clear-confirm-message =
    { $count ->
        [one] 表示中のログファイル ({ $count } 件) が完全に削除されます。
       *[other] 表示中のログファイル ({ $count } 件) が完全に削除されます。
    }
