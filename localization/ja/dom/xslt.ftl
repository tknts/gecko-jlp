...
xslt-bad-recursion = XSLT スタイルシートが (おそらく) 無限ループしていると思われます。
...
xpath-invalid-axis = XPath 構文解析エラー: 無効な軸指定 (axis) が無効です:
...
xslt-load-blocked-error = セキュリティー上の理由により XSLT スタイルシートの読み込みはブロックされました。
xpath-invalid-expression-evaluated = 評価する式が不正です。
xpath-unbalanced-curly-brace =‘{’ と ‘}’ の対応が正しくありません。
...
xslt-unknown-error = 不明なエラーが発生しました ({ $errorCode })
...
xslt-loading-error = スタイルシートの読み込みエラーです: { $error }
