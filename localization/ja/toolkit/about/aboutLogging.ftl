...
about-logging-preset-vpn-description = IP Protection (VPN) の問題を診断するためのログモジュール
...
# Variables:
#   $errorText (string) - The received error message, inserted as is.
about-logging-profile-storage-error = アップロードされたプロファイルの保存中にエラーが発生しました: { $errorText }
# Variables:
#   $errorText (string) - The received error message, inserted as is.
...
about-logging-uploaded-profiles-title = アップロード済みプロファイル
about-logging-no-uploaded-profiles = まだプロファイルはアップロードされていません。
...
about-logging-delete-profile-confirm = プロファイル “{ $profileName }”を削除してもよろしいですか? この操作は元に戻せません。
about-logging-deleting-profile = 削除しています…
