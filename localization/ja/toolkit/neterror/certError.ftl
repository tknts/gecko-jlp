...
fp-certerror-illustration-alt =
  .alt = オレンジ色の警告サインを見つめているキツネのイラスト

fp-certerror-why-site-dangerous = このサイトが危険と判断された理由はなんですか?
fp-certerror-what-can-you-do = どのような対処ができますか?

fp-certerror-advanced-title = 詳細情報

fp-certerror-advanced-button = 詳細設定
fp-certerror-hide-advanced-button = 詳細を隠す
...
fp-certerror-override-exception-button-2 = { $hostname } へ進む (危険)
    .accesskey = P
fp-certerror-intro = { -brand-short-name } は <strong>{ $hostname }</strong> で深刻なセキュリティ上の問題が発生している可能性があることを検出しました。サイトになりすました第三者が、クレジットカード情報やパスワード、メールなどを盗み取ろうとしている可能性があります。
fp-certerror-expired-into = { -brand-short-name } は <strong>{ $hostname }</strong> でセキュリティ上の問題を検出しました。サイトの設定が正しくないか、お使いのデバイスの時計 (日付・時刻) が誤っている可能性があります。
fp-certerror-transparency-intro = <strong>{ $hostname }</strong> になりすました第三者が、クレジットカード情報やパスワード、メールなどを盗み取ろうとしている可能性があります。
