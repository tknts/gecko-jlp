...
neterror-dns-not-found-hint-firewall-2 = { -brand-short-name } にウェブへのアクセス権限があるか確認してください (接続されていても、ファイアウォールでブロックされている可能性があります)。
...
# Variables:
#   $hostname (String) - Hostname of the website to which the user was trying to connect.
fp-neterror-invalid-header-value-intro = <strong>{ $hostname }</strong> から返されたヘッダーに、Web セキュリティ標準で許可されていない空文字が含まれています。
fp-neterror-content-encoding-intro = 表示しようとしているページは、無効またはサポートされていない圧縮形式が使用されているため表示できません。
fp-neterror-coop-coep-intro = セキュリティ設定が前のページと一致しないため、{ -brand-short-name } はこのページを読み込みませんでした。
fp-neterror-blocked-by-policy-intro = 組織のポリシーにより、このページまたはウェブサイトへのアクセスがブロックされました。
...
fp-neterror-denied-port-access = このアドレスは、通常のウェブ閲覧以外に使用されるネットワークポートを利用しています。{ -brand-short-name } は、ユーザー保護のためこのリクエストを中止しました。
