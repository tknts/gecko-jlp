...
videocontrols-error-src-not-supported =  動画のフォーマットか MIME タイプに対応していません。
videocontrols-error-no-source = 対応するフォーマットと MIME タイプの動画が見つかりません。
videocontrols-error-generic =  不明なエラーにより動画の再生が中止されました。
